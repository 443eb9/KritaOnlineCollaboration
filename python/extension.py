...
class OnlineCollabExtension(Extension):
    def __init__(self, parent):
        super().__init__(parent)

        ext = {"windows": ".dll", "linux": ".so", "macos": ".dylib"}[platform]
        lib_dir = Path(__file__).parent / "lib"
        lib_file = lib_dir / f"kritaonlinecollab{ext}"
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to load OnlineCollab library from {lib_file}: {e}{deps}"
            )
        finally:
...
Krita.instance().addExtension(OnlineCollabExtension(Krita.instance()))